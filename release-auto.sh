#!/bin/bash

# Automated Release Script for Cloud Ops JDBC Driver
# This script handles version incrementing and deployment

set -e  # Exit on error

echo "Cloud Ops JDBC Driver - Automated Release"
echo "================================================="

# Colors for output
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Function to increment version
increment_version() {
    local version=$1
    local release_type=$2
    
    # Split version into array
    IFS='.' read -ra VERSION_PARTS <<< "$version"
    
    case $release_type in
        major)
            ((VERSION_PARTS[0]++))
            VERSION_PARTS[1]=0
            VERSION_PARTS[2]=0
            ;;
        minor)
            ((VERSION_PARTS[1]++))
            VERSION_PARTS[2]=0
            ;;
        patch)
            ((VERSION_PARTS[2]++))
            ;;
    esac
    
    echo "${VERSION_PARTS[0]}.${VERSION_PARTS[1]}.${VERSION_PARTS[2]}"
}

# Parse command line arguments
RELEASE_TYPE=""
RELEASE_METHOD=""
while [[ $# -gt 0 ]]; do
    case $1 in
        --patch)
            RELEASE_TYPE="patch"
            shift
            ;;
        --minor)
            RELEASE_TYPE="minor"
            shift
            ;;
        --major)
            RELEASE_TYPE="major"
            shift
            ;;
        --maven-release)
            RELEASE_METHOD="maven"
            shift
            ;;
        --manual)
            RELEASE_METHOD="manual"
            shift
            ;;
        --help)
            echo "Usage: $0 [OPTIONS]"
            echo "Options:"
            echo "  --patch          Patch release (bug fixes)"
            echo "  --minor          Minor release (new features)"
            echo "  --major          Major release (breaking changes)"
            echo "  --maven-release  Use maven-release-plugin"
            echo "  --manual         Manual release"
            echo "  --help           Show this help message"
            exit 0
            ;;
        *)
            echo "Unknown option: $1"
            exit 1
            ;;
    esac
done

# Check if we're on a clean working directory
if [[ -n $(git status -s) ]]; then
    echo -e "${RED}Error: Working directory has uncommitted changes${NC}"
    echo "Please commit or stash your changes before releasing"
    exit 1
fi

# Get current version
CURRENT_VERSION=$(mvn help:evaluate -Dexpression=project.version -q -DforceStdout | grep -v '\[')
echo -e "Current version: ${BLUE}$CURRENT_VERSION${NC}"

# Remove -SNAPSHOT if present
BASE_VERSION=${CURRENT_VERSION%-SNAPSHOT}

# Ask for release type if not provided
if [[ -z "$RELEASE_TYPE" ]]; then
    echo -e "\n${YELLOW}Select release type:${NC}"
    echo "1) Patch release (bug fixes)"
    echo "2) Minor release (new features, backward compatible)"
    echo "3) Major release (breaking changes)"
    echo "4) Custom version"
    read -p "Enter choice (1-4): " choice

    case $choice in
        1)
            RELEASE_TYPE="patch"
            ;;
        2)
            RELEASE_TYPE="minor"
            ;;
        3)
            RELEASE_TYPE="major"
            ;;
        4)
            read -p "Enter custom version: " NEW_VERSION
            RELEASE_TYPE="custom"
            ;;
        *)
            echo -e "${RED}Invalid choice${NC}"
            exit 1
            ;;
    esac
fi

# Calculate new version based on release type
if [[ "$RELEASE_TYPE" != "custom" ]]; then
    NEW_VERSION=$(increment_version $BASE_VERSION $RELEASE_TYPE)
fi

echo -e "\nWill release version: ${GREEN}$NEW_VERSION${NC}"

# Only ask for confirmation in interactive mode
if [[ -z "$RELEASE_METHOD" ]]; then
    read -p "Continue? (y/n): " -n 1 -r
    echo
    
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Release cancelled"
        exit 0
    fi
fi

# Build Calcite dependencies first
echo -e "\n${YELLOW}Building Calcite dependencies...${NC}"
cd ../calcite
./gradlew assemble || echo "Calcite build had issues, but JARs might exist"
cd ../cloud-ops-jdbc-driver

# Option 1: Using maven-release-plugin (recommended)
use_release_plugin() {
    echo -e "\n${YELLOW}Using maven-release-plugin...${NC}"
    
    # Create a release.properties file for non-interactive mode
    cat > release.properties << EOF
scm.tag=cloud-ops-jdbc-driver-${NEW_VERSION}
project.rel.com.kenstott.components\\:cloud-ops-jdbc-driver=${NEW_VERSION}
project.dev.com.kenstott.components\\:cloud-ops-jdbc-driver=$(increment_version $NEW_VERSION patch)-SNAPSHOT
EOF
    
    # Prepare the release (updates versions, creates tag)
    echo -e "${YELLOW}Preparing release...${NC}"
    mvn release:prepare \
        -B \
        -DreleaseVersion=$NEW_VERSION \
        -DdevelopmentVersion=$(increment_version $NEW_VERSION patch)-SNAPSHOT \
        -DautoVersionSubmodules=true \
        -Dresume=false \
        -DskipTests=true \
        -Darguments="-DskipTests" || {
            echo -e "${RED}Release preparation failed. Cleaning up...${NC}"
            mvn release:rollback
            rm -f release.properties pom.xml.releaseBackup
            exit 1
        }
    
    # Perform the release (builds and deploys)
    echo -e "${YELLOW}Performing release deployment...${NC}"
    mvn release:perform \
        -B \
        -Darguments="-DskipTests -P release" || {
            echo -e "${RED}Release deployment failed. The tag has been created but deployment failed.${NC}"
            echo "You can retry deployment with: mvn release:perform"
            exit 1
        }
    
    # Clean up
    rm -f release.properties pom.xml.releaseBackup
}

# Option 2: Manual version update and deploy
manual_release() {
    echo -e "\n${YELLOW}Performing manual release...${NC}"
    
    # Update version
    mvn versions:set -DnewVersion=$NEW_VERSION
    mvn versions:commit
    
    # Clean and test
    mvn clean test
    
    # Deploy
    mvn clean deploy -P release
    
    # Create git tag
    git add pom.xml
    git commit -m "Release version $NEW_VERSION"
    git tag -a "cloud-ops-jdbc-driver-$NEW_VERSION" -m "Release version $NEW_VERSION"
    
    # Update to next development version
    NEXT_VERSION=$(increment_version $NEW_VERSION patch)-SNAPSHOT
    mvn versions:set -DnewVersion=$NEXT_VERSION
    mvn versions:commit
    git add pom.xml
    git commit -m "Prepare for next development iteration"
}

# Ask which method to use (if not provided via command line)
if [[ -z "$RELEASE_METHOD" ]]; then
    echo -e "\n${YELLOW}Select release method:${NC}"
    echo "1) Use maven-release-plugin (recommended)"
    echo "2) Manual release"
    read -p "Enter choice (1-2): " method_choice
    
    case $method_choice in
        1)
            RELEASE_METHOD="maven"
            ;;
        2)
            RELEASE_METHOD="manual"
            ;;
        *)
            echo -e "${RED}Invalid choice${NC}"
            exit 1
            ;;
    esac
fi

# Execute the chosen release method
case $RELEASE_METHOD in
    maven)
        use_release_plugin
        ;;
    manual)
        manual_release
        ;;
    *)
        echo -e "${RED}Invalid release method${NC}"
        exit 1
        ;;
esac

echo -e "\n${GREEN}Release complete!${NC}"
echo -e "Version ${GREEN}$NEW_VERSION${NC} has been deployed to Maven Central"
echo -e "\nNext steps:"
echo -e "1. Push commits and tags: ${YELLOW}git push origin main --tags${NC}"
echo -e "2. Check deployment at: ${BLUE}https://central.sonatype.com/artifact/com.kenstott.components/cloud-ops-jdbc-driver/$NEW_VERSION${NC}"
echo -e "3. Maven Central sync takes ~30 minutes"